    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#000', position: 'relative' }}>
      ...
      <div style={{ flex: 1, position: 'relative', overflow: 'hidden', minHeight: 0 }}>
        <img
          ...
          alt="Camera preview"
          style={{ width: '100%', height: '100%', objectFit: 'cover', filter: filterStyle, opacity: intensity / 100 * (1 - (100 - intensity) / 100) + (100 - intensity) / 100 }}
        />

        {/* Top controls */}
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 52, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', padding: '0 16px', justifyContent: 'space-between' }}>
          ...
          <div style={{ padding: '2px 6px', border: '1px solid #5FB98F', background: 'rgba(95,185,143,0.2)' }}>
        ...
        {mode === 'STOP-MOTION' && stopFrames.length > 0 && (
          <div style={{ position: 'absolute', top: 56, left: 8, right: 8, display: 'flex', gap: 4, overflowX: 'auto' }}>
            {stopFrames.map(f => (
              <div
                key={f}
                style={{ flexShrink: 0, width: 48, height: 48, background: 'rgba(26,26,26,0.85)', border: '1px solid #3C3C3C', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
              >
        ...
        {mode === 'AR' && (
          <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ position: 'relative', width: 200, height: 200 }}>
              ...
              <div style={{ position: 'absolute', inset: 40, border: '2px solid rgba(58,143,222,0.8)' }} />
              <div style={{ position: 'absolute', bottom: 16, right: 16, background: 'rgba(58,143,222,0.15)', border: '1px solid #3A8FDE', padding: '4px 8px' }}>
      ...
      <div style={{ background: 'rgba(0,0,0,0.88)', paddingTop: 8 }}>
        <div style={{ display: 'flex', gap: 8, padding: '0 8px 8px', overflowX: 'auto' }}>
          {FILTERS.map((f, i) => (
            ...
            >
              <div style={{ width: 56, height: 56, border: `1px solid ${activeFilter === i ? '#3A8FDE' : '#3C3C3C'}`, overflow: 'hidden' }}>
                <img
              ...
              <span style={{ fontSize: 9, color: activeFilter === i ? '#3A8FDE' : '#8E8E8E' }}>{f.name}</span>
            </button>
          ))}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '0 16px 8px' }}>
          ...
          <input
            ...
            style={{ flex: 1 }}
          />
          <span style={{ ...M, fontSize: 10, color: '#E4E4E4', width: 24, textAlign: 'right' }}>{intensity}</span>
        </div>
      </div>

      {/* Mode selector */}
      <div style={{ background: 'rgba(0,0,0,0.9)', padding: '8px 0' }}>
        <div style={{ display: 'flex', justifyContent: 'center', gap: 24, overflowX: 'auto', padding: '0 16px' }}>
          {MODES.map(m => (
            <button
              key={m}
              onClick={() => setMode(m)}
              style={{ fontSize: 11, letterSpacing: '0.06em', color: mode === m ? '#3A8FDE' : '#8E8E8E', background: 'none', border: 'none', cursor: 'pointer', flexShrink: 0, fontWeight: mode === m ? 500 : 400 }}
            >
              {m}
            </button>
          ))}
      ...
      {/* Controls */}
      <div style={{ background: 'rgba(0,0,0,0.95)', display: 'flex', alignItems: 'center', justifyContent: 'space-around', padding: '12px 24px' }}>
        ...
        >
          {mode === 'VÍDEO' ? (
            <div style={{ width: recording ? 20 : 48, height: recording ? 20 : 48, background: '#D25252', borderRadius: recording ? 2 : '50%', transition: 'all 0.2s' }} />
          ) : mode === 'STOP-MOTION' ? (
        ...
        <button onClick={() => navigate('projects')} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4 }}>
      ...
      {mode === 'STOP-MOTION' && (
        <div style={{ background: 'rgba(0,0,0,0.95)', display: 'flex', alignItems: 'center', gap: 8, padding: '6px 16px 10px' }}>
          <span style={{ fontSize: 10, color: '#8E8E8E', whiteSpace: 'nowrap' }}>Taxa de reprodução</span>
          <input type="range" min={1} max={24} value={stopFps} onChange={e => setStopFps(Number(e.target.value))} style={{ flex: 1 }} />
          <span style={{ ...M, fontSize: 10, color: '#E4E4E4', whiteSpace: 'nowrap' }}>{stopFps} fps</span>
        </div>
      )}
    </div>
  );
