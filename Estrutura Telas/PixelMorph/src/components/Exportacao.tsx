...
export default function Exportacao({ onFechar }: Props) {
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.6)' }} onClick={onFechar} />
      <div style={{
        position: 'relative',
        height: '85%',
        background: '#252525',
        ...
        {/* Header */}
        <div style={{ display: 'flex', alignItems: 'center', padding: '0 16px 12px', borderBottom: '1px solid #3C3C3C' }}>
        ...
        {estado === 'config' && (
          <>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
              {aba === 'rapido' && (
                <>
                  <div style={{ color: '#8E8E8E', fontSize: 11, letterSpacing: 1, marginBottom: 8 }}>FORMATO</div>
                  ...
                  <div style={{ color: '#8E8E8E', fontSize: 11, letterSpacing: 1, marginBottom: 8 }}>QUALIDADE</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                    <input
                      type="range" min={10} max={100} value={qualidade}
                      onChange={e => setQualidade(+e.target.value)}
                      style={{ flex: 1, accentColor: '#3A8FDE', height: 2 }}
                    />
                    <span className="mono" style={{ color: '#E4E4E4', fontSize: 13, width: 40, textAlign: 'right' }}>{qualidade}%</span>
                  </div>
                  <div style={{ color: '#8E8E8E', fontSize: 11, marginBottom: 16 }}>
                    Tamanho estimado: <span className="mono" style={{ color: '#E4E4E4' }}>{tamanhoEstimado} KB</span>
                  </div>

                  <div style={{ color: '#8E8E8E', fontSize: 11, letterSpacing: 1, marginBottom: 8 }}>TAMANHO</div>
                  <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap' }}>
                    {tamanhos.map(t => (
                      <button
                        key={t}
                  ...
                  <div style={{ color: '#8E8E8E', fontSize: 11, letterSpacing: 1, marginBottom: 8 }}>PREDEFINIÇÕES DE REDE SOCIAL</div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                    {redesSociais.map(r => (
                      ...
                      >
                        ...
                        <span className="mono" style={{ color: '#8E8E8E', fontSize: 11 }}>{r.res}</span>
                      </div>
                    ))}
                  </div>
                </>
              ...
              {aba === 'profissional' && (
                <>
                  <div style={{ color: '#8E8E8E', fontSize: 11, letterSpacing: 1, marginBottom: 8 }}>FORMATO PROFISSIONAL</div>
                  <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
                    {['PSD', 'TIFF'].map(f => (
                      <div key={f} style={{
                        flex: 1, padding: 16, background: '#2F2F2F', border: '1px solid #3C3C3C',
                        display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, cursor: 'pointer',
                      }}>
                        <span style={{ color: '#3A8FDE', fontSize: 20, fontWeight: 700 }}>{f}</span>
                        <span style={{ color: '#8E8E8E', fontSize: 11 }}>Adobe {f === 'PSD' ? 'Photoshop' : 'TIFF'}</span>
                      </div>
                    ))}
                  </div>

                  <div style={{ background: '#2F2F2F', border: '1px solid #3C3C3C', padding: 12, marginBottom: 12 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                      <input type="checkbox" checked={camadas} onChange={e => setCamadas(e.target.checked)} style={{ accentColor: '#3A8FDE' }} />
                      <span style={{ color: '#E4E4E4', fontSize: 14 }}>Manter camadas</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <input type="checkbox" checked={mascaras} onChange={e => setMascaras(e.target.checked)} style={{ accentColor: '#3A8FDE' }} />
                      <span style={{ color: '#E4E4E4', fontSize: 14 }}>Manter máscaras</span>
                    </div>
                  </div>

                  <div style={{ color: '#8E8E8E', fontSize: 11, letterSpacing: 1, marginBottom: 8 }}>DPI</div>
                  ...
                  <div style={{ color: '#8E8E8E', fontSize: 11, letterSpacing: 1, marginBottom: 8 }}>ESPAÇO DE COR</div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 12 }}>
                    {['sRGB', 'Adobe RGB', 'DCI-P3'].map(ec => (
                      <label key={ec} style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                        <input type="radio" name="ecor" value={ec} checked={espacoCor === ec} onChange={() => setEspacoCor(ec)} style={{ accentColor: '#3A8FDE' }} />
                        <span style={{ color: '#E4E4E4', fontSize: 14 }}>{ec}</span>
                      </label>
                    ))}
                  </div>

                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#2F2F2F', border: '1px solid #3C3C3C', padding: 12 }}>
                    <Switch label="Simulação CMYK" value={cmyk} onChange={setCmyk} />
                  </div>
                  {cmyk && (
                    <div style={{ padding: '8px 12px', background: 'rgba(210,160,94,0.12)', border: '1px solid #D2A05E', marginTop: 4 }}>
                      <span style={{ color: '#D2A05E', fontSize: 12 }}>⚠ CMYK pode alterar cores. Revise antes de imprimir.</span>
                    </div>
                  )}
                </>
              )}
              ...
              {aba === 'marcadagua' && (
                <>
                      ...
                      {Array.from({ length: 9 }).map((_, i) => (
                        <div
                          ...
                          style={{
                            ...
                            cursor: 'pointer',
                            border: posicaoMarca === i ? '2px solid #3A8FDE' : '1px solid rgba(255,255,255,0.1)',
                          }}
                        >
                          {posicaoMarca === i && (
                            <div style={{
                              background: 'rgba(0,0,0,0.7)',
                              color: '#E4E4E4', fontSize: 10, padding: '2px 4px',
                  ...
                  {[
                    { label: 'Opacidade', val: opacidadeMarca, set: setOpacidadeMarca, unit: '%', min: 0, max: 100 },
                    { label: 'Tamanho', val: tamanhoMarca, set: setTamanhoMarca, unit: 'px', min: 10, max: 100 },
                    { label: 'Rotação', val: rotacaoMarca, set: setRotacaoMarca, unit: '°', min: -180, max: 180 },
                  ].map(({ label, val, set, unit, min, max }) => (
                    <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                      <span style={{ color: '#8E8E8E', fontSize: 12, width: 70, flexShrink: 0 }}>{label}</span>
                      <input
                        type="range" min={min} max={max} value={val}
                        onChange={e => set(+e.target.value)}
                        style={{ flex: 1, accentColor: '#3A8FDE' }}
                      />
                      <span className="mono" style={{ color: '#E4E4E4', fontSize: 12, width: 40, textAlign: 'right' }}>{val}{unit}</span>
                    </div>
                  ))}

                  <div style={{ marginTop: 12 }}>
                    <div style={{ color: '#8E8E8E', fontSize: 11, letterSpacing: 1, marginBottom: 6 }}>LINK DO QR CODE</div>
                    <div style={{ position: 'relative' }}>
                      <input
                        type="text"
                        placeholder="https://joaosilva.photo"
                        style={{
                          width: '100%', background: '#2F2F2F', border: '1px solid #3C3C3C',
                          color: '#E4E4E4', fontSize: 13, padding: '8px 12px',
                  ...
                  <button style={{
                    ...
                    color: '#E4E4E4', fontSize: 13, padding: '8px 0', cursor: 'pointer',
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 32, gap: 24 }}>
            <div style={{ width: 80, height: 80, position: 'relative' }}>
              <svg viewBox="0 0 80 80" style={{ width: '100%', height: '100%' }}>
                ...
                <circle
                  cx={40} cy={40} r={36} fill="none" stroke="#3A8FDE" strokeWidth={4}
                  strokeDasharray={226}
              ...
              <span className="mono" style={{
                ...
                color: '#E4E4E4', fontSize: 18,
              }}>{progresso}%</span>
            </div>
            <div style={{ textAlign: 'center' }}>
              <div style={{ color: '#E4E4E4', fontSize: 15 }}>Exportando...</div>
              <div style={{ color: '#8E8E8E', fontSize: 12, marginTop: 4 }}>Ensaio Praia 04.{formato.toLowerCase()}</div>
            </div>
          </div>
        )}

        {estado === 'concluido' && (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: 24, gap: 0, overflowY: 'auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
              <div style={{ width: 40, height: 40, background: '#5FB98F', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Check size={24} color='#fff' />
              </div>
              <div>
                <div style={{ color: '#E4E4E4', fontSize: 15, fontWeight: 500 }}>Exportação concluída</div>
            ...
            <div style={{ color: '#8E8E8E', fontSize: 11, letterSpacing: 1, marginBottom: 10 }}>COMPARTILHAR</div>
            <div style={{ display: 'flex', gap: 12 }}>
              {[
                ...
                <div key={label} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, cursor: 'pointer' }}>
                  <div style={{ width: 44, height: 44, background: '#2F2F2F', border: '1px solid #3C3C3C', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <button
              ...
              style={{
                marginTop: 'auto',
                width: '100%', background: '#3A8FDE', border: 'none',
                color: '#fff', fontSize: 15, fontWeight: 600, padding: '13px 0', cursor: 'pointer',
              }}
            >Concluir</button>
          </div>
        )}
